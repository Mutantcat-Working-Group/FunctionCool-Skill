...
- 📦 **体积小** —— 整包 28KB，比一张截图还小。
- ⚡ **直接可用** —— 复制到 `~/.claude/skills/functioncool/` 后，模型自动识别。
...
1. **剥离 `code` 字段**
   脚本 `query.sh` 在拿到 API 返回后会**主动删掉 `code` 字段**。模型只看到"方法长什么样"，看不到"方法怎么写"——这强制模型自己写实现，避免照抄，也避免把大段源码塞进上下文。
...
4. **零本地状态**
   没有数据库、没有索引文件、没有 vector store。**28KB 全是元数据**——SKILL.md 告诉模型"什么时候用、怎么用"，`query.sh` 帮它调一次 HTTP。这就是为什么能"无需下载、体积小、直接可用"。
...
MIT · 由 [Mutantcat Working Group](https://www.mutantcat.org) 维护。
